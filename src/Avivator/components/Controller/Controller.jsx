...
const Controller = () => {
    ...
    const [channelOptions, useColormap, isChannelLoading, setIsChannelLoading, removeIsChannelLoading, pixelValues, isViewerLoading] = useViewerStore(store => [store.channelOptions, store.useColormap, store.isChannelLoading, store.setIsChannelLoading, store.removeIsChannelLoading, store.pixelValues, store.isViewerLoading], shallow);
    ...
    const channelControllers = ids.map((id, i) => {
        const onSelectionChange = e => {
            ...
            const selection = {
                ...selections[i], c: channelOptions.indexOf(e.target.value)
            };
        ...
        const handleSliderChange = (e, v) => setPropertiesForChannel(i, {contrastLimits: v});
    ...
    return (
        <Menu>
            <TabPanel value={0} index={0}>
                ...
                {globalControllers}
                {!isViewerLoading && !isRgb ? (<Grid container>{channelControllers}</Grid>) : (
