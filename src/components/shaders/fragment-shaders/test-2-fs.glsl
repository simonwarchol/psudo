...
//    return color * intensity;
        vec3 lrgb = lrgb_to_xyz(srgb_to_lrgb(color) * intensity);
        return lrgb;

}

vec3 color_transfer_srgb(vec3 color, float intensity) {
    vec3 oklab = color_transfer(color, intensity);
...
void mutate_color(inout vec3 rgb, float intensity0, float intensity1, float intensity2, float intensity3, float intensity4, float intensity5, vec2 vTexCoord){
    vec3 lab = color_transfer(colors[0], intensity0);
    lab += color_transfer(colors[1], intensity1);
    lab += color_transfer(colors[2], intensity2);
    lab += color_transfer(colors[3], intensity3);
    lab += color_transfer(colors[4], intensity4);
    lab += color_transfer(colors[5], intensity5);
    rgb = lrgb_to_srgb(xyz_to_lrgb(lab));
    //    vec3 xyz = oklab_to_xyz(lab);
    //    rgb = xyz_to_lrgb(xyz);
    //    rgb = lrgb_to_srgb(xyz_to_lrgb(oklab_to_xyz(lab)));
