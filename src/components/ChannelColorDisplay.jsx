...
import {useChannelsStore, useLoader, useViewerStore} from "../Avivator/state.js";
import shallow from 'zustand/shallow';

import _ from 'lodash'
import {getSingleSelectionStats, truncateDecimalNumber} from "../Avivator/viewerUtils.js";
...
function ChannelColorDisplay(props) {
    ...
    const [channelsVisible, contrastLimits, colors, domains, selections, ids, setPropertiesForChannel, toggleIsOnSetter, removeChannel] = useChannelsStore(store => [store.channelsVisible, store.contrastLimits, store.colors, store.domains, store.selections, store.ids, store.setPropertiesForChannel, store.toggleIsOn, store.removeChannel], shallow);
    const [source, channelOptions, useColormap, isChannelLoading, setIsChannelLoading, removeIsChannelLoading, pixelValues, isViewerLoading] = useViewerStore(store => [store.source, store.channelOptions, store.useColormap, store.isChannelLoading, store.setIsChannelLoading, store.removeIsChannelLoading, store.pixelValues, store.isViewerLoading], shallow);
    ...
    const handleSliderChange = (e, v) => setPropertiesForChannel(ind, {contrastLimits: v});
    ...
    const onSelectionChange = e => {
        ...
        const selection = {
            ...selections[ind], c: channelOptions.indexOf(e.target.value)
        };
        ...
        getSingleSelectionStats({
            ...
            setPropertiesForChannel(ind, {
                contrastLimits: newContrastLimit, domains: domain
            });
    ...
    return (
        <>
            {colors?.[channelIndex] !== undefined && (
                <>
                    <Grid container sx={{'height': '100%'}} direction="row" justifyContent="flex-start"
                          alignItems="center" ref={ref}>
                        <Select native value={channelOptions[selections[ind].c]} onChange={onSelectionChange}
                                variant="standard" className={'simon'}>
                        ...
                        {channelsVisible?.[channelIndex] && (
                            ...
                            <Grid item xs={4} sx={{'paddingLeft': '15px', 'paddingTop': '20px'}}>
                                <ColorNameSelect label={'Color Name'} channelIndex={channelIndex} multiSelect={false}/>
                            </Grid>
                        )}
                        {channelsVisible?.[channelIndex] && (

                            <Grid item xs={2} sx={{'paddingLeft': '10px', 'paddingTop': '25px'}}>
                                <Button aria-controls={openEl ? 'basic-menu' : undefined}
                                        aria-haspopup="true"
                                ...
                                >
                                    <ChromePicker disableAlpha={true} color={pickerColor}
