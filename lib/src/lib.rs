...
fn annealing(intensities: &Vec<f32>) -> Result<Vec<f32>, Error> {
    ...
    Ok(intensities.clone()) // Return the best parameters
}

// let best_params = annealing(&float_color_map);
// best_params.unwrap()
// 1.0
// float_color_map
// float_color_map
...
// best_params
#[wasm_bindgen]
pub fn optimize(colors: &[u16]) -> f32 {
    ...
    let float_color_map: Vec<f32> = colors
        ...
        .collect::<Vec<f32>>();
    // let test = annealing(&float_color_map).unwrap();
    // for &color in colors {
    //     console::log_1(&JsValue::from(color));
    // }
    // take sum of colors
