...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var i : u32 = 0u; i < lengths.colors_length / 3u; i = i + 1u) {
        ...
        let srgb_result = vec3<f32>(lrgb_to_srgb(xyz_to_lrgb(oklab_to_xyz(color_scaled_by_intensity))));
        pixel_color = vec4<f32>(pixel_color[0] + srgb_result[0], pixel_color[1] + srgb_result[1], pixel_color[2] + srgb_result[2], 1.0f);
    }
    output.data[index] = pixel_color;
}
